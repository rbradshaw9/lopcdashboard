<?php
/**
 * ⚠️ BACKUP FILE - DO NOT USE IN PRODUCTION ⚠️
 * 
 * Apple-Inspired LearnDash Student Dashboard
 * Professional, mobile-responsive dashboard with advanced features
 * 
 * VERSION: 3.0 - Complete Overhaul (STABLE) - BACKUP COPY
 * BACKED UP: October 27, 2025
 * ORIGINAL UPDATE: October 2025
 * 
 * THIS IS A BACKUP OF THE FULL-FEATURED DASHBOARD
 * Working file: function lop_ultimate_dashboard_shortcod.php
 * 
 * FEATURES:
 ...
 * - Professional Apple-inspired design with unified CSS
 * - Mobile-responsive with dark mode support
 * - Announcements system with membership targeting
 * - Banner and collapsible announcement support
 */
...
function lop_apple_dashboard_shortcode() {
    ...
    ?>
        ...
        <section class="lop-stats-grid" aria-label="Learning Statistics">
            ...
            <?php
            // Determine font size based on number length for responsive display
            $points_str = (string) $positivity_points_display;
            $points_length = strlen( $points_str );
            if ( $points_length <= 3 ) {
                $font_size = '2.5rem'; // 0-999
            } elseif ( $points_length <= 4 ) {
                $font_size = '2.2rem'; // 1,000-9,999
            } elseif ( $points_length <= 5 ) {
                $font_size = '2rem'; // 10,000-99,999
            } else {
                $font_size = '1.8rem'; // 100,000+
            }
            ?>
            <div class="lop-stat-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; cursor: pointer;" onclick="window.location.href='<?php echo esc_url( site_url( '/positivity-points/' ) ); ?>'">
                <div class="lop-stat-icon" style="background: rgba(255,255,255,0.2); color: white;">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"></polygon>
                    </svg>
                </div>
                <div class="lop-stat-number" style="color: white; font-size: <?php echo $font_size; ?>;"><?php echo number_format( $positivity_points_display ); ?></div>
                <div class="lop-stat-label" style="color: rgba(255,255,255,0.9); font-weight: 600;">Total Points</div>
            </div>

            <div id="streak-card" class="lop-stat-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border: none;">
                <div class="lop-stat-icon" style="background: rgba(255,255,255,0.2); color: white;">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"></path>
                    </svg>
                </div>
                <div id="streak-number" class="lop-stat-number" style="color: white;"><?php echo absint( $streak_display ); ?></div>
                <div class="lop-stat-label" style="color: rgba(255,255,255,0.9); font-weight: 600;">Day Streak</div>
            </div>
        </section>
        ...
        <!-- Achievements Section (GamiPress Only) -->
        <?php 
        // Get ALL GamiPress achievements (get total count first, then recent ones for display)
        $all_user_achievements = array();
        $user_achievements = array();
        if ( function_exists( 'gamipress_get_user_achievements' ) ) {
            // Get total count (no limit)
            $all_user_achievements = gamipress_get_user_achievements( array(
                'user_id' => $user_id,
            ...
            // Get recent ones for display (limit to 12)
        ...
        // Define color schemes for different achievement types
        $achievement_colors = array(
        ...
        // Only show section if user has GamiPress achievements
        if ( ! empty( $user_achievements ) ) : 
            $total_achievements = count( $all_user_achievements ); // Use total count, not limited results
            $display_count = min( 3, count( $user_achievements ) ); // Show only 3 achievements from recent results
        ?>
        <section class="lop-section" aria-label="Your Achievements">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--lop-space-lg);">
                <h2 class="lop-section-title" style="margin-bottom: 0;">
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <a href="https://members.liveonpurposecentral.com/game-on-copy/" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; padding: var(--lop-space-sm) var(--lop-space-md); border-radius: var(--lop-radius-md); font-weight: 600; font-size: 0.875rem; transition: all 0.2s; box-shadow: var(--lop-shadow-sm); display: inline-flex; align-items: center; gap: 0.5rem;" onmouseover="this.style.transform='translateY(-1px)'; this.style.boxShadow='var(--lop-shadow-md)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='var(--lop-shadow-sm)';">
                    View All <?php echo $total_achievements; ?> Achievements
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="m9 18 6-6-6-6"/>
                    </svg>
                </a>
            </div>
            <div style="display: flex; flex-wrap: wrap; gap: var(--lop-space-md);">
                
                <?php // Display only first 3 GamiPress Achievements
                if ( ! empty( $user_achievements ) ) :
                    $displayed_count = 0;
                    foreach ( $user_achievements as $achievement ) :
                        ...
                        $achievement_content = get_post_field( 'post_content', $achievement_id );
                        ...
                        $date_earned = get_post_meta( $achievement_id, '_gamipress_achievement_date_earned_' . $user_id, true );
                        
                        // Get achievement type for color scheme
                        $achievement_type = get_post_type( $achievement_id );
                        $color_scheme = isset( $achievement_colors[$achievement_type] ) ? $achievement_colors[$achievement_type] : $achievement_colors['default'];
                        
                        // Get points awarded for this achievement
                        $points_awarded = 0;
                        if ( function_exists( 'gamipress_get_post_meta' ) ) {
                            $points_awarded = gamipress_get_post_meta( $achievement_id, '_gamipress_points', true );
                        }
                        if ( empty( $points_awarded ) ) {
                            $points_awarded = get_post_meta( $achievement_id, '_gamipress_points', true );
                        }
                ?>
                    <div class="achievement-card" style="flex: 1; min-width: 250px; max-width: 400px; background: <?php echo esc_attr( $color_scheme['gradient'] ); ?>; color: white; border-radius: var(--lop-radius-md); padding: var(--lop-space-lg); box-shadow: var(--lop-shadow-md); position: relative; overflow: hidden;">
                        ...
                        <div style="display: flex; align-items: flex-start; gap: var(--lop-space-md); margin-bottom: var(--lop-space-sm); position: relative; z-index: 2;">
                            <?php if ( $achievement_image ) : ?>
                                <img src="<?php echo esc_url( $achievement_image ); ?>" alt="<?php echo esc_attr( $achievement_title ); ?>" style="width: 48px; height: 48px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.3); flex-shrink: 0; object-fit: cover;">
                            <?php else : ?>
                                <div style="width: 48px; height: 48px; background: rgba(255,255,255,0.2); border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; border: 2px solid rgba(255,255,255,0.3);">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"></polygon>
                                    </svg>
                                </div>
                            ...
                            <div style="flex: 1; min-width: 0;">
                                <h3 style="margin: 0 0 var(--lop-space-xs) 0; font-size: 1.125rem; font-weight: 600; line-height: 1.3; color: white;"><?php echo esc_html( $achievement_title ); ?></h3>
                                <?php if ( $points_awarded > 0 ) : ?>
                                    <div style="display: inline-flex; align-items: center; gap: 0.25rem; background: rgba(255,255,255,0.2); padding: 0.25rem 0.5rem; border-radius: 12px; font-size: 0.75rem; font-weight: 600; margin-bottom: var(--lop-space-xs);">
                        ...
                        <p style="margin: 0 0 var(--lop-space-md) 0; opacity: 0.9; font-size: 0.875rem; line-height: 1.4; position: relative; z-index: 2;">
                            <?php echo $achievement_excerpt ? esc_html( $achievement_excerpt ) : 'Achievement unlocked! 🏆'; ?>
                        </p>
                        
                        <?php if ( $date_earned ) : ?>
                            <div style="font-size: 0.75rem; opacity: 0.8; display: flex; align-items: center; gap: 0.25rem; position: relative; z-index: 2;">
                                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <circle cx="12" cy="12" r="10"></circle>
                                    <polyline points="12,6 12,12 16,14"></polyline>
                                </svg>
